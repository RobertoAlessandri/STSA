...
[sig_m, Fs] = audioread('source_SuzanneVega_vocal.wav');
sig_m = sig_m(:, 1);
...
for n=0:n_frames-1
    ...
    P_X(P_X == 0) =  P_X_min; % avoids numerical issues when P_x==0
    N = length(X_n);

    % Gain Estimation
    Z_B = (P_X) - a * (P_v.^c);
    Z_B(Z_B<0) = 0; % This should be considered as the Power Spectrum of the "clean "
                    % the "clean " signal, so as Power Spectrum, it can't
